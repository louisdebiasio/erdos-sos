...
structure FaceFrame (host : FiniteHypergraph Host) (face remaining : List Host) : Prop where
  enumeration : (face ++ remaining).Perm host.vertices

theorem FaceFrame.fullWord_nodup (host : FiniteHypergraph Host) (face remaining : List Host)
    (frame : FaceFrame host face remaining) (state : MarkedCut Host)
    (ordering : state.word.Perm remaining) : (face ++ state.word).Nodup :=
  ((ordering.append_left face).trans frame.enumeration).nodup_iff.mpr host.vertices_nodup

def FaceInShadow (host : FiniteHypergraph Host) (face : List Host) : Prop :=
  ∃ marker, host.HasEdge (face ++ [marker])

def MarkedSupport (target : FiniteHypergraph Target) (host : FiniteHypergraph Host)
    (face : List Target) (faceMap : Target → Host) (endpoint : Target)
    (state : MarkedCut Host) : Prop :=
  ∃ embedding : Embedding target host, embedding.Anchored face faceMap ∧
    embedding.toFun endpoint = state.marker ∧
    ∀ vertex, vertex ∈ target.vertices →
      embedding.toFun vertex ∈ face.map faceMap ++ state.prefixList

def SupportsAt (target : FiniteHypergraph Target) (host : FiniteHypergraph Host)
    (face : List Target) (faceMap : Target → Host) (endpoint : Target)
    (state : MarkedCut Host) : Prop :=
  host.HasEdge (face.map faceMap ++ [state.marker]) ∧
    MarkedSupport target host face faceMap endpoint state

def MarkedFamily (target : FiniteHypergraph Target) (host : FiniteHypergraph Host)
    (face : List Target) (faceMap : Target → Host) (endpoint : Target) (remaining : List Host) :=
  {state : MarkedCut Host // state.word.Perm remaining ∧ SupportsAt target host face faceMap endpoint state}

def ShadowOrderings (host : FiniteHypergraph Host) (face remaining : List Host) :=
  {word : List Host // word.Perm remaining ∧ FaceInShadow host face}
