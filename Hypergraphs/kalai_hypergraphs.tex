...
Section~\ref{sec:paths-r} gives a self-contained warm-up for tight paths
in arbitrary uniformity, with a $3$-uniform worked example.
Sections~\ref{sec:anchored} and
...
\section{Warm-up: tight paths}
\label{sec:paths-r}

We first prove the theorem for tight paths in arbitrary uniformity.
The main point is to use a path that avoids the vertex at the current
marked position, so that this vertex can extend the path.  An
$r$-cycle of permutation entries then puts the extended path in the
required form.  Branching will require a stronger support condition
in Section~\ref{sec:anchored}.

\subsection{Marked permutations}

Fix $r\geq2$ and an $r$-graph $H$ on $n\geq r$ vertices.  For an
$(r-1)$-set $F$, write
\[
 d_H(F)=|\{z\notin F:F\cup\{z\}\in E(H)\}|.
\]
For a permutation $\pi=(v_1,\ldots,v_n)$ and $r\leq i\leq n$, call
$(\pi,i)$ \emph{marked} if
$\{v_1,\ldots,v_{r-1},v_i\}\in E(H)$.  Its prefix
...
\begin{equation}
 v_1,\ldots,v_{r-1},y_r,\ldots,y_{k+r-1}.
 \label{eq:rooted-path}
\end{equation}
The starting $(r-1)$-tuple is ordered, but the remaining path vertices
need not occur in the permutation in path order.  Support is an existence
...
\end{equation}
Let $\cQ$ be the set of permutations whose first $(r-1)$-set belongs to
$\partial H$.  Then
\begin{equation}
 Q:=|\cQ|=(r-1)!\,|\partial H|(n-r+1)!.
 \label{eq:Q}
\end{equation}
\end{lemma}

\begin{proof}
Every marked state supports a one-edge path starting at its first
$(r-1)$-tuple, so $\cA_1=\cO(H)$.  For each fixed ordering of an
$(r-1)$-set $F$, there are $(n-r+1)!$ permutations starting with that
ordering, and each contributes $d_H(F)$ marks.  Each edge contains
$r$ such sets, each with $(r-1)!$ orders, giving
\[
 M=(r-1)!(n-r+1)!\sum_{F\in\partial H}d_H(F)
   =r!\,e(H)(n-r+1)!.
\]
Each shadow set has $(r-1)!$ orders and each such order can begin
$(n-r+1)!$ permutations.  This proves \eqref{eq:Q}.
\end{proof}
...
\]
Such a permutation has a marked position, so its first $(r-1)$-set
lies in $\partial H$.  Hence $|\cD_k|\leq Q$.

We construct an injection $\Phi_k:\cA_k\setminus\cD_k\to\cA_{k+1}$.
Take a state $(\pi,i)$ in its domain and write
\[
 \pi=(a_1,\ldots,a_{r-1},W,z,R),\qquad
 W=(v_r,\ldots,v_{i-1}),\quad z=v_i,\quad
 R=(v_{i+1},\ldots,v_n).
\]
Since $i>i_0(\pi)$, the prefix $S_{i_0(\pi)}(\pi)$ contains a path
\[
 a_1,\ldots,a_{r-1},y_r,\ldots,y_{k+r-1}
\]
and does not contain $z$.  The marking edge
$\{z,a_1,\ldots,a_{r-1}\}$ extends this to a path
\begin{equation}
 z,a_1,\ldots,a_{r-1},y_r,\ldots,y_{k+r-1}
 \label{eq:extended-path}
\end{equation}
with $k+1$ edges.  Indeed, its first edge is the marking edge and its
remaining consecutive $r$-sets are precisely the old path edges.
All vertices remain distinct.

Define
\begin{equation}
 \Phi_k\bigl((a_1,\ldots,a_{r-1},W,z,R),i\bigr)
       =\bigl((z,a_1,\ldots,a_{r-2},W,a_{r-1},R),i\bigr).
 \label{eq:cycle-map}
\end{equation}
The new prefix has the same vertex set.  Its first $(r-1)$-tuple is
$(z,a_1,\ldots,a_{r-2})$, the starting tuple of
\eqref{eq:extended-path}, and its last vertex is $a_{r-1}$.
Thus its marking edge is still $\{z,a_1,\ldots,a_{r-1}\}$.
The image is therefore in $\cA_{k+1}$.

The retained index $i$ identifies the displaced entry $a_{r-1}$.
For any image $(\rho,i)$, the unique possible source ordering is
\begin{equation}
 (\rho_2,\ldots,\rho_{r-1},\rho_i,\rho_r,\ldots,\rho_{i-1},\rho_1,
       \rho_{i+1},\ldots,\rho_n).
 \label{eq:inverse}
\end{equation}
This inverse cycle on positions $1,\ldots,r-1,i$ recovers the source
without using the earlier index $i_0(\pi)$ or choosing a path witness.
Hence $A_k-|\cD_k|\leq A_{k+1}$ proves \eqref{eq:transfer}.
When $r=2$, the tuples indexed from $1$ to $r-2$ or from $2$ to
$r-1$ are empty, and the map simply exchanges the first entry and
the marked entry.
\end{proof}

\begin{theorem}[Tight-path shadow bound]
\label{thm:path-shadow}
For all integers $r\geq2$ and $m\geq1$ and every
$P_m^{(r)}$-free $r$-graph $H$,
\begin{equation}
 r e(H)\leq(m-1)|\partial H|.
...
the ordered tuple $(v_1,\ldots,v_{r-1})$.  The mark supplies an edge
through that tuple, but this edge need not belong to the supported path.
The cycle in \eqref{eq:cycle-map} preserves the marking edge and puts
the extended path's starting tuple first.  The map does not depend on
which path is chosen; the earlier marked position ensures that a path
avoiding $z$ exists.

\begin{example}[The map in uniformity three]
Let $r=3$.  For five distinct vertices $a,b,c,d,z$, write $abc$ for
...
An edgeless host has $A_k=M=Q=0$.  For $n=r$, every permutation has at
most one mark, so every supporting pair is discarded.
For the complete $r$-graph on $s=m+r-2\geq r$ vertices, there is no
$P_m^{(r)}$, and
\[
 r\binom{s}{r}=(m-1)\binom{s}{r-1}.
\]
Thus the coefficient in the shadow bound cannot be reduced.  More
precisely, every permutation has $s-r+1$ marks and exactly $s-k-r+2$
of them support a $k$-edge path, for $1\leq k\leq m$.  Hence
$A_k=(s-k-r+2)s!$ and $A_k-A_{k+1}=Q=s!$ throughout the nontrivial
